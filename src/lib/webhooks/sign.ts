...
//
// Every delivery carries an `X-ChatChacha-Signature` header so receivers
// can verify the request really came from Chat Chacha and wasn't tampered
// with or replayed. The scheme is Stripe-style:
//
//   X-ChatChacha-Signature: t=<unix_seconds>,v1=<hex HMAC-SHA256>
//
...
/**
 * Build the `X-ChatChacha-Signature` header value for `rawBody`, signed
...
/**
 * Verify a signature header. Exposed so an integration between two
 * Chat Chacha instances (or a test) can validate deliveries; receivers in other stacks
